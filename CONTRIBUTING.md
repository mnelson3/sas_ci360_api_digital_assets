 ...
 * `develop` - active development; open pull requests against this branch.
 * `staging` - release candidates staged for validation before going out.
 * `main` - the latest released, production-ready code.

Changes flow `develop` -> `staging` -> `main`.
